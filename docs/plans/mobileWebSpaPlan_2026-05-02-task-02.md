...
Tauri `invoke` 없이 HTTP API를 호출하는 클라이언트 레이어를 작성한다.
모든 모듈은 `src/lib/connect.ts`에서 URL과 토큰을 읽어 인증 헤더를 붙인다.

### connect.ts — 연결 설정 저장소
...
Task 01 완료 후 시작 (`mobile-spa/` 프로젝트 구조 필요).

## Verification

```bash
# TypeScript 타입 체크 — 타입 에러 0개
...
- `ApiError` 클래스가 `instanceof` 체크에 의존 — 번들러에 따라 prototype chain 문제 발생 가능. `err.status` 체크로 대체 가능.
